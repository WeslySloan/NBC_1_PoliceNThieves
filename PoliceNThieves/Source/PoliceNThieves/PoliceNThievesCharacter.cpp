...
{
	// 1. 필요한 변수 선언 및 초기화 (맨 위로 이동)
	FHitResult HitResult;
	FVector StartLocation = GetActorLocation();
	FVector EndLocation = StartLocation + GetActorForwardVector() * AttackRange;

	FCollisionQueryParams Params;
	Params.AddIgnoredActor(this); // Trace가 자기 자신을 무시하도록 설정

	// 2. 공격 판정 실행 (Line Trace)
	// LineTraceSingleByChannel 함수는 성공 시 true, 실패 시 false를 반환하며, 
	// 이 결과를 bHit에 저장합니다.
	bool bHit = GetWorld()->LineTraceSingleByChannel(
		HitResult,
		StartLocation,
		EndLocation,
		ECC_Pawn, // 충돌 채널은 Pawn (캐릭터)를 사용합니다.
		Params
	);
	...
	// =======================================================
	// [공격 판정 및 결과 처리] - 오직 서버에서만 실행됩니다.
	// =======================================================

	// 4. 디버그 라인 출력 (bHit 값이 결정된 후에 호출되어야 함)
	DrawDebugLine(
		GetWorld(),
		StartLocation,
		EndLocation,
		bHit ? FColor::Red : FColor::Green, // bHit 값에 따라 색상 결정
		false,
		3.0f,
		0,
		5.0f
	...
	if (bHit)
	{
		...
		if (HitCharacter)
		{
			// 로그를 통해 공격이 다른 캐릭터에 명중했는지 확인합니다.
			UE_LOG(LogTemp, Warning, TEXT("SERVER HIT: 다른 캐릭터 명중! 이름: %s"), *HitActor->GetName());

			// TODO: 여기에 게임 규칙에 따른 로직을 호출합니다. (예: 체포, 목숨 차감)
			// UGameplayStatics::ApplyDamage(HitActor, 10.0f, GetController(), this, UDamageType::StaticClass());
		}
	}
	else
	{
		UE_LOG(LogTemp, Log, TEXT("SERVER: 공격 빗나감."));
	}
	// =======================================================
}
